...
$message="";   
    ...
    $iner=$connection->insert($con,"storeowner", $_POST["ID"],$_POST["fname"],$_POST["lname"], $_POST['contact'],$_POST['nid'],$_POST['email'], $_POST['password'], $_POST['gender'],
   $_POST['Address']);
    if($iner==true)
    {
    ...
    $existingdata = file_get_contents('../json/stinfo.json');
        $tempJSONdata = json_decode($existingdata, true);
        $tempJSONdata[] =$formdata;
        $jsondata = json_encode($tempJSONdata, JSON_PRETTY_PRINT);

        if(file_put_contents("../json/stinfo.json", $jsondata)){
            $_SESSION['message']='Data successfully saved.';
            header("Location:../view/stOwnReg.php");
        } else{
